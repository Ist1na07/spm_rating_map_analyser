...
### v0.5.1
- **性能优化**：整图分析（键型分类 + 难度评级）平均耗时 641ms → 188ms（3.4 倍），最差谱面 13.6s → 1.2s（11.7 倍，LN 密集马拉松图不再卡顿）
- 难度评级：锁手/盾牌/Inverse 预计算由 O(n²) 改为按列二分窗口；平滑/插值例程改为双指针；Xbar 距离权重查表化；小节数据单遍扫描
- 键型分类引擎：全图参考中位数整图只算一次（原为每段重复全图排序）；网格拟合与 LN 窗口扫描改为二分定界
- 输出与 v0.5.0 **逐位一致**（332 图全量回归：SR、分段、标签、sort、置信度无任何差异）
